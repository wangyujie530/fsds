  ...
  <div class="nixs">
    ...
    <van-popup v-model="show" position="bottom" :style="{ height: '30%' }">
      <div v-for="(ites, indes) in listssd" :key="indes">
        <div
          v-show="item.isov"
          v-for="(item, index) in ites.foods"
          :key="index"
          class="xia"
        >
          <div class="fonsd">
...
export default {
  ...
  data() {
    return {
      ...
      show: false,
      lists: this.$store.state.list.goods[0],
      isover: false,
      texst: "￥20元起送",
    };
  },
  ...
  methods: {
    ...
    jiajia(v) {
      ...
      this.texst = "立即结算";
      this.$store.state.list.goods[0].foods[v].isov = true;
      this.isover = true;
    ...
    jianjian(v) {
      if (this.lists.foods[v].sellCounts < 1) {
        ...
        this.$store.state.list.goods[0].foods[v].isov = false;
        this.texst = "￥20元起送";
      } else {
        this.$store.state.zons--;
        this.lists.foods[v].sellCounts--;
